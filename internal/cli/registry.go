...
func commandDefinitions() []*Command {
	...
	commands := []*Command{
		...
		alias(&Command{Name: "auth.login", Path: pathWithFlint("auth", "login"), Description: "Sign in through the Flint website (recommended for local development)", Local: true, MCPHidden: true, Arguments: []Arg{{Name: "no_open", Flag: "--no-open", Type: "boolean", Description: "Print the sign-in link without opening a browser"}}, Examples: ex("flint login", "flint auth login", "flint auth login --no-open", "flint auth login --live --profile production")}, "login"),
