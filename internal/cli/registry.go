...
func commandDefinitions() []*Command {
	...
	commands := []*Command{
		...
		{Name: "auth.import", Path: pathWithFlint("auth", "import"), Description: "Import and validate a Flint API key", Local: true, MCPHidden: true, Arguments: []Arg{{Name: "stdin", Flag: "--stdin", Type: "boolean", Description: "Read the key from stdin"}}, Examples: ex("flint auth import --stdin < token.txt")},
		...
		alias(&Command{Name: "auth.logout", Path: pathWithFlint("auth", "logout"), Description: "Remove the active profile credential", Local: true, Destructive: true, Examples: ex("flint auth logout --confirm")}, "logout"),
