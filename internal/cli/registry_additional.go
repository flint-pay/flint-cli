...
func additionalCommands(page []Arg, input Arg) []*Command {
	...
	return []*Command{
		...
		apiCommand("invoices.late-fees.assess", "Assess the invoice's frozen late fee policy", "POST", "/v1/invoices/{invoice_id}/late-fees", "assessInvoiceLateFee", "AssessInvoiceLateFeeRequest", "InvoiceResponse", []Arg{pos("invoice_id", "inv_", "Flint invoice ID"), idFlag("invoice_schedule_entry_id", "invse_", "invoice_schedule_entry_id", "", "Schedule entry ID for a scheduled invoice", false), input}, true, false, false, false, "flint invoices late-fees assess inv_123"),
		apiCommand("invoices.late-fees.waive", "Waive an invoice late fee's unpaid remainder", "POST", "/v1/invoices/{invoice_id}/late-fees/{invoice_late_fee_id}/waive", "waiveInvoiceLateFee", "WaiveInvoiceLateFeeRequest", "InvoiceResponse", []Arg{pos("invoice_id", "inv_", "Flint invoice ID"), {Name: "invoice_late_fee_id", Type: "string", Required: true, Positional: 2, AcceptsHistoryRef: true, IDPrefix: "inlf_", Description: "Invoice late fee ID"}, flag("reason", "string", "reason", "", "Reason for waiving the fee", true), input}, true, false, false, false, "flint invoices late-fees waive inv_123 inlf_123 --reason courtesy"),
